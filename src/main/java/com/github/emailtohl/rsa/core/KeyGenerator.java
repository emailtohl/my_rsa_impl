...
/**
 * <p>
 * RSA算法需要6个参数，p,q,n,φ(n),e,d ，其中： p，q是随机生成的大素数，n = p * q ，是加密和解密计算时的模。φ(n)（用fn表示）是n的欧拉函数值。
 * </p>
 * <p>
 * 在数论中，对正整数n的欧拉函数是小于n的数中与n互素的数的个数（包括1），例如φ(8) = 4，这是因为1,3,5,7这4个数和8互素。e是加密时中使用的公钥，它与φ(n)互素。
 * </p>
 * <p>
 * d是解密时需要的私钥，它是通过e和φ(n)计算出来的，它们的关系是：e * d ≡ 1 (mod φ(n))，即： e*d % φ(n) = 1，其中“%”是模运算符，这个等式还可以写成：e*d = 1 + u*φ(n)
 * </p>
 * <p>
 * 设m是数字化的明文，注意，这里的m一定要比n小！ c是加密后的值，加密和解密的过程实际上进行幂模运算，过程如下： 加密：c = m^e % n 解密：m = c^d % n
 * </p>
 *
 * @author HeLei
 */
public final class KeyGenerator {
		...
		if (ZERO.equals(fn.remainder(e))) {
			...
			if (ZERO.equals(fn.remainder(e))) {
				...
				if (ZERO.equals(fn.remainder(e))) {
					...
					if (ZERO.equals(fn.remainder(e))) {
						LOG.severe("Create a RSA parameter failure, re execution");// 如果模n与上面的素数都不互素的话，创建RSA参数失败
	...
	private static boolean test(BigInteger e, BigInteger d, BigInteger n) {
		...
		LOG.finest("test m = " + mm);
		//BigInteger cc = mm.modPow(e, n);
		BigInteger cc = powModByMontgomery(mm, e, n);
		LOG.finest("test c = " + cc);
		//BigInteger dm = cc.modPow(d, n);
		BigInteger dm = powModByMontgomery(cc, d, n);
		LOG.finest("test dm = " + dm);
		if (mm.compareTo(dm) != 0) {
			LOG.finest("Test failed");
			return false;
		} else {
			LOG.finest("Test passed ");
		}
		return true;
	}

	/**
	 * 幂模运算也遵循乘法分配率，所以对于大数的幂模运算，可以先将底数做模运算后，再做指数运算，这样可以将数值运算保持在较小的数域范围内。 蒙哥马利算法计算大数的幂模运算的思路是不断将指数进行除2分解，直到指数分解到1为止，当然每次分解指数时，同时也在计算底数的平方。
	 *
	...
	private static class Euclid {
		...
		private void generateD() {
			...
			LOG.finest("gcd = " + gcd);// 先打印查看最大公约数是否为1，保证无异常
			LOG.finest("x = " + x);
			LOG.finest("y = " + y);
			/*
			...
			d = y;
			LOG.finest("d = " + d);
			LOG.finest("e * d (mod φ(n)) = " + (e.multiply(d).remainder(fn)));
		}

		/**
		 * 下面的方法使用了扩展欧几里得算法获取逆元，用于RSA中获取密钥d所用 注意：通过扩展欧几里得算法得到的x,y可能为负数，最后要把它加上模的倍数，直到成正数
		 *
